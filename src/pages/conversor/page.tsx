...
export function Conversor() {
  ...
  const convertCurrencies = async (event: SubmitEvent) => {
    ...
    if (data && otherCurrenciesData) {
      const mergedData = JSON.parse(data)
        .concat(JSON.parse(otherCurrenciesData))
        .concat({
          moneda: "ARS",
          casa: "Banco Central",
      ...
      const firstCurrencyData = mergedData.find(
        (currency: priceAPIResponse) => currency.nombre === firstCurrency
      );
      const secondCurrencyData = mergedData.find(
        (currency: priceAPIResponse) => currency.nombre === secondCurrency
      );

      if (
        firstCurrencyData &&
        secondCurrencyData &&
        valueToConvertRef.current
      ) {
        const inputValue = parseFloat(valueToConvertRef.current.value); // Convertimos el valor a número
        if (!isNaN(inputValue)) {
          const conversionRate =
            (firstCurrencyData.venta / secondCurrencyData.venta) * inputValue;
  ...
  return (
    <main className={container}>
      ...
      <div className={outputContainer}>
        $ {output.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ".")}
      </div>
    </main>
  );
}
